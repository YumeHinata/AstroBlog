...
幻梦开启Openclash的Fake-ip模式，同时通过UU加速器插件加速设备上的游戏时，会发现游戏流量走了Openclash而不是加速器。那这就很讨厌了，互联网上也有很多人遇到这个问题，但是大多是调整Openclash的规则后掩盖问题。那么有可能通过规则方式把流量抢回UU加速器插件吗，我其实已经见到过一个方案，但是幻梦好像没法直接使用，于是利用另一个项目剩余的一点token，让DS写了这个脚本。

**测试系统**：istoreOS 24.10.7
...
[下载uu_router.sh](http://fastly.jsdelivr.net/gh/YumeHinata/uugamebooster-docker@main/uu_route.sh "uu_router.sh")

## 使用说明：

之前可以用命令行控制安装、卸载的。现在我们可以直接通过内置的引导提示来完成安装工作，更加的简单。你只需要`./uu_router.sh`运行即可。

脚本运行后会自动监控uu加速插件的行为，然后调整规则，把流量从Openclash手里抢回来。
