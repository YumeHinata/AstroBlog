...
幻梦开启Openclash的Fake-ip模式，同时通过UU加速器插件加速设备上的游戏时，会发现游戏流量走了Openclash而不是加速器。那这就很讨厌了，互联网上也有很多人遇到这个问题，但是大多是调整Openclash的规则后掩盖问题。那么有可能通过规则方式把流量抢回UU加速器插件吗，幻梦其实已经见到过一个方案，但是好像没法直接使用，于是利用另一个项目剩余的一点token，让DS写了这个脚本。
